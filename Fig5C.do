...
log using "Fig5C.log", replace

* X and Y missing dependent on C, C missing dependent on Y
...
/* Scenario Rx dep on C, Rc dep on Y, RY dep on C*/
centile c, centile(50)
gen try1=r(c_1)
centile y, centile(50)
gen try3=r(c_1)
centile c, centile(70)
gen try2=r(c_1)

gen psely=0.9 if c<=try1
replace psely=0.1 if c>try1

gen pselx=0.8 if c>try2
replace pselx=0.4 if c<=try2

gen pselc=0.9 if y<=try3
replace pselc=0.1 if y>try3
...
mi impute chained (regress)  c=y x, add(20) 
...
gen trythis11=M[1,1]
replace micoefyxceyx=trythis11 if id==`sim'
...
gen trythis11=M[2,1]
replace semicoefyxceyx=trythis11 if id==`sim'
...
gen biasmisubxy=micoefyxceyx-`xcoef'
...
gen inyxmisubxyy=(micoefyxceyx-1.96*semicoefyxceyx)<`xcoef'&`xcoef'<(micoefyxceyx+1.96*semicoefyxceyx)
